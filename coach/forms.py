...
class GpxUploadForm(forms.Form):
    gpx_files = MultipleFileField(help_text="Select one or more .gpx files exported from Zepp, Strava, Garmin, etc.")
...
class ZeppCsvImportForm(forms.Form):
    csv_file = forms.FileField(
        help_text="CSV with columns like: date, hrv, rhr, sleep_hours",
        widget=forms.ClearableFileInput(attrs={"class": INPUT_CLS}),
    )
    date_col = forms.CharField(initial="date", widget=forms.TextInput(attrs={"class": INPUT_CLS}))
    hrv_col = forms.CharField(initial="hrv", required=False, widget=forms.TextInput(attrs={"class": INPUT_CLS}))
    rhr_col = forms.CharField(initial="rhr", required=False, widget=forms.TextInput(attrs={"class": INPUT_CLS}))
    sleep_col = forms.CharField(initial="sleep_hours", required=False, widget=forms.TextInput(attrs={"class": INPUT_CLS}))
